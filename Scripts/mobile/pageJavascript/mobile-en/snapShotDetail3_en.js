            ...
            if (dt.HttpStatus == 200 && dt.HttpData.data) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    if (result[i].bConfirmed == false) {
                        ...
                        strData += '<li class="accordion-item">' + '<a href="#" class="item-link item-content">' + '	<div class="item-inner">' + '		<div class="item-title-row">' + '			<div class="item-subtitle">' + formatDate(result[i].Time) + '</div>' + '			<div class="item-after">' + isSureSpan + '</div>' + '		</div>' + '		<div class="item-text">' + result[i].EventMsg + '</div>' + '	</div>' + '</a>' + '<div class="accordion-item-content content-container">' + '<div class="content-container-block">' + '<p>时间：' + formatDate(result[i].Time) + '</p>' + '<p>事件：' + textareaEventMsg + '</p>' + '<p>处理意见：<textarea class="advice-textarea" placeholder="请输入处理意见"></textarea></p>' + '<p>是否发送短信：&nbsp;&nbsp;<label class="toggle toggle-init color-blue" onclick="onProcsCheckBox(' + countNum + ')">' + '<input type="checkbox" class="isProcsInput"><span class="toggle-icon"></span></label><div class="procsContent list-block" style="height:auto;display:block"></div></p>' + "<p><a href='#' class=\"button button-big button-fill color-blue\" onclick='OnSureMessage(\"" + countNum + "\",\"" + textareaEventMsg + "\",\"" + result[i].Time + "\")' values='" + result[i] + "' title=\"" + result[i].User_Confirmed + formatDate(result[i].Dt_Confirmed) + "\">确定</a></p>" + '</div></div>' + '</li>';
                        countNum++;
                    } else {
                        isSureSpan = "<span class='span-color-sure sure-flag'>Confirmed</span>";
                        strSureData += '<li class="accordion-item">' + '<a href="#" class="item-link item-content">' + '	<div class="item-inner">' + '		<div class="item-title-row">' + '			<div class="item-subtitle">' + formatDate(result[i].Time) + '</div>' + '			<div class="item-after">' + isSureSpan + '</div>' + '		</div>' + '		<div class="item-text">' + result[i].EventMsg + '</div>' + '	</div>' + '</a>' + '<div class="accordion-item-content content-container">' + '<div class="content-container-block">' + '<p>时间：' + formatDate(result[i].Time) + '</p>' + '<p>事件：' + textareaEventMsg + '</p>' + '<p>处理意见：' + textareaAdviceMsg + '</p>' + "<p>确认人：" + result[i].User_Confirmed + '</p><p>确认时间：' + formatDate(result[i].Dt_Confirmed) + "</p>" + '</div></div>' + '</li>';
                    }
                }
